...
import { extractMedicationsFromPdf, type ExtractionResult } from "@/lib/actions/medication-import";
import { useToast } from "@/components/Toast";
import { ImportReview } from "./ImportReview";
import type { MedicationCategory, MedicationField } from "@/lib/supabase/types";

// Entry point for the "ייבוא מ-PDF" tab: upload a dosing-table PDF, send it
// for AI extraction (lib/actions/medication-import.ts), then hand the
// result to ImportReview for the admin to edit/approve before anything is
// written to the database. See the delivery plan for why extraction never
// writes directly -- this is clinical dosing data and needs a human check.
export function ImportPanel({
  ...
  function handleExtract() {
    ...
    startExtracting(async () => {
      try {
        const extraction = await extractMedicationsFromPdf(sectionSlug, uploaded.storagePath);
  ...
  return (
    <div className="flex max-w-lg flex-col gap-3">
      <p className="text-sm text-neutral-500 dark:text-neutral-400">
        העלאת קובץ PDF עם טבלת מינונים -- המערכת תנתח אותו ותציע שורות תרופה
        למיפוי לשדות הקיימים (ותציע שדות חדשים במידת הצורך), לפני שתבחר
        לשמור בפועל.
      </p>
      ...
      {!uploaded ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            ...
            >
              <Sparkles size={16} />
              {extracting ? "מנתח... (יכול לקחת עד דקה)" : "ניתוח PDF"}
            </button>
            <button
              type="button"
              disabled={extracting}
